...
def sideview(rw = 5,largeprob = 0.3,spiral_on = True, throwmode = 'drag', gridL = 300,itts = 100):
    p0 =  largeprob
    mode = throwmode
    w = rw
    ...
    for i in range(itts):
        ...
        for j in saverange:
            ...
            k = j
            x = a.plot_pile()
            pre = 'spiral50_in4'+str(spiral_on)+'_sideview_'+mode+'_p'+str(p0)[-1]+'_t'+str(a.t)+'_w'+str(w)+'_kim'
            np.save(pre+str(i) ,x)#[130:])
        # a.run(200)
        # x = a.plot_pile()
        ...
        # np.save(pre + 'rwindow'+str(i),rwindow)
